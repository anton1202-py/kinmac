...
class Action(models.Model):
    ...
    period = models.IntegerField(verbose_name='Период для запроса Excel файла',
                                 null=True, blank=True)
...
class ArticleForAction(models.Model):
    ...
    action = models.ForeignKey(Action, on_delete=models.CASCADE,
                               verbose_name='Акция', related_name='article_for_action')
    article = models.ForeignKey(Articles, on_delete=models.CASCADE,
                                verbose_name='Артикул', related_name='article_in_action')
    in_action = models.BooleanField(verbose_name='Участвует в акции',
                                    null=True, blank=True)
    action_price = models.FloatField(verbose_name='Цена в акции',
                                     null=True, blank=True)
    current_price = models.FloatField(verbose_name='Текущая цена',
                                      null=True, blank=True)
    discount = models.IntegerField(verbose_name='Текущая скидка',
                                   null=True, blank=True)
    plan_discount = models.IntegerField(verbose_name='Рекомендуемая скидка для участия в акции',
                                        null=True, blank=True)

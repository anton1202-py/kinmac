...
from api_requests.wb_requests import (
    wb_action_details_info, wb_actions_first_list, wb_articles_in_action)
from celery_tasks.celery import app

from action.models import Action, ArticleForAction
from action.supplyment import action_data_from_front, add_article_may_be_in_action, get_excel_data_from_front
from kinmac.constants_file import (
    TELEGRAM_ADMIN_CHAT_ID, bot, wb_headers, wb_header_with_lk_cookie, actions_info_users_list)
...
def add_new_actions_wb_to_db():
    ...
    if actions_not_exist_str:
        ...
        actions_details = wb_action_details_info(
            wb_headers, actions_not_exist_str)
        ...
        if actions_details and 'data' in actions_details:
            for detail in actions_details['data']['promotions']:
                ...
                search_params = {'marketplace': CodingMarketplaces.objects.get(
                    marketpalce='Wildberries'), 'action_number': detail['id']}
                values_for_update = {
                    'name': detail['name'],
                    'period': action_front_info[detail['id']] if action_front_info else None,

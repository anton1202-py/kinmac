...
from api_requests.wb_requests import (
    create_excel_file_with_article_in_auto_actions, take_excel_file_data_in_auto_actions, wb_action_info_from_front)
    ...
    if article_action_data:
        ...
        for data in article_action_data:
            if Articles.objects.filter(nomenclatura_wb=data['id']).exists():
                ...
                if ArticleForAction.objects.filter(action=action_obj,
                                                   article=article_obj).exists():
                    ArticleForAction.objects.filter(action=action_obj,
...
def get_excel_data_from_front(action_number, action_period):
    ...
    create_excel_file_with_article_in_auto_actions(
        wb_header_with_lk_cookie, action_period)
    # Получаем зашифрованный excel файл
    excel_data = take_excel_file_data_in_auto_actions(
        wb_header_with_lk_cookie, action_number, action_period)
    if excel_data:
        ...
        excel_data = pd.DataFrame(read_excel_file, columns=[
        ...
        action_discount_list = excel_data['Загружаемая скидка для участия в акции'].to_list(
        )
        ...
        for i, article in enumerate(wb_article_list):
            if Articles.objects.filter(
                ...
                action_discount = (current_seller_price -
                                   action_price) / current_seller_price * 100
...
def action_data_from_front():
    ...
    action_data = wb_action_info_from_front(
        wb_header_with_lk_cookie, date_from, date_to)
    if action_data:

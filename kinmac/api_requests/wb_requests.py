...
def wb_article_data_from_api(
    header, update_date=None, mn_id=0, common_data=None, counter=0
):
    """Получаем данные всех артикулов в ВБ"""
    if not common_data:
        common_data = []
    if update_date:
        cursor = {"limit": 100, "updatedAt": update_date, "nmID": mn_id}
    else:
    ...
    if response.status_code == 200:
        ...
        if len(json.loads(response.text)["cards"]) == 100:
            # time.sleep(1)
            return wb_article_data_from_api(
                header,
                check_amount["updatedAt"],
                check_amount["nmID"],
                common_data,
                counter,
            )
        ...
        return wb_article_data_from_api(
            header, update_date, mn_id, common_data, counter
        )
    else:
...
def wb_price_discount_info(
    header, limit=1000, offset=0, counter=0, article_adv_data=None
):
    """
    ...
    if response.status_code == 200:
        ...
        if len(articles_list) == limit:
            ...
            return wb_price_discount_info(
                header, limit, offset, counter, article_adv_data
            )
        else:
...
def get_check_storage_cost_report_status(header, report_id):
    ...
    """
    print("get_check_storage_cost_report_status", datetime.now().time())
    print(report_id)
...
def get_adv_campaign_lists_data(header):
    ...
    сгруппированных по типу и статусу, с информацией
    о дате последнего изменения кампании.
...
def wb_articles_in_action(
    ...
    if response.status_code == 200:
        ...
        if len(articles_list) == limit:
            offset = limit * counter
            return wb_articles_in_action(
                header, action_number, limit, offset, counter, article_adv_data
            )
        else:
            return article_adv_data
    else:

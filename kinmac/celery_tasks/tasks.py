...
def add_data_stock_api():
    try:
        ...
        bot.send_message(chat_id=CHAT_ID, text=f'Ошибка выполнения задачи ADD_DATA_STOCK_API: {e}')
        raise e # повторное возбуждение исключения


@app.task
def add_data_sales():
    try:
        control_date_sales = date.today() - timedelta(days=0)
        ...
        # обработка ошибки и отправка сообщения через бота
        bot.send_message(chat_id=CHAT_ID, text=f'Ошибка выполнения задачи ADD_DATA_SALES_API: {e}')
        raise e # повторное возбуждение исключения
...
def add_stock_data_site():
    try:
        ...
        # обработка ошибки и отправка сообщения через бота
        bot.send_message(chat_id=CHAT_ID, text=f'Ошибка выполнения задачи ADD_DATA_STOCK_SITE: {e}')
        raise e # повторное возбуждение исключения

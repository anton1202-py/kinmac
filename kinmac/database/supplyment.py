...
class OzonSalesDataSave:
    ...
    def _get_or_create_article(
        self, article_info: dict, article_obj=None
    ) -> Articles:
        ...
        if OzonProduct.objects.filter(sku=sku):
            article_obj = OzonProduct.objects.filter(sku=sku).first()
        elif OzonProduct.objects.filter(seller_article=offer_id):
            article_obj = OzonProduct.objects.filter(
                seller_article=offer_id
            ).first()
        elif OzonProduct.objects.filter(barcode=barcode):
            article_obj = OzonProduct.objects.filter(barcode=barcode).first()
        elif OzonProduct.objects.filter(name=name):
            article_obj = OzonProduct.objects.filter(name=name).first()

        # if not article_obj:
        #     print(name, offer_id)
        #     article_obj = OzonProduct.objects.create(
    ...
    def save_realization_report(
        ...
        realization_obj, created = (
            RealizationReportOzon.objects.update_or_create(
                company=company,
    ...
    def save_article_in_realization_report(
        ...
        delivery_commission: dict = article_report_data.get(
            "delivery_commission"
        )
            ...
            article_realization_obj, created = (
                ArticlesRealizationReportOzon.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "commission_ratio": article_report_data.get(
                            "commission_ratio"
                        ),
                    },
                )
    ...
    def _save_article_return_comission(
        self,
        report_row: ArticlesRealizationReportOzon,
        return_commission: dict,
    ) -> None:

        ReturnArticlesRealizationReportOzon.objects.update_or_create(
            report_row=report_row,
            defaults={
                "price_per_instance": return_commission.get(
                    "price_per_instance"
                ),
                ...
                "bank_coinvestment": return_commission.get(
                    "bank_coinvestment"
                ),

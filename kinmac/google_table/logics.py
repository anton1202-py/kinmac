...
class WbMarketplaceArticlesData:
    ...
    def only_sales_data(self):
        ...
        sale_data = (
            ...
            .values("nm_id")
            .annotate(
                sales_amount=Count("retail_amount"),
                sales_sum=Sum("retail_price_withdisc_rub"),
            )
        )
...
class WbAnalyticalTableData:
    ...
    def daily_orders_data(self):
        orders = (
            ...
            .order_by("supplier_article")
            .annotate(order_day=TruncDate("order_date"))
            .values("order_day", "supplier_article")
            .annotate(total_count=Count("id"), total_sum=Sum("finish_price"))
            .annotate(average_price=F("total_sum") / F("total_count"))
    ...
    def daily_stock_data(self):
        latest_stocks = (
            StocksSite.objects.filter(
                pub_date__gte=self.start_date, pub_date__lte=self.end_date
            )
            .annotate(
        ...
        final_stocks = StocksSite.objects.filter(
            pub_date__in=latest_stocks.values("latest_pub_date"),
            seller_article__in=latest_stocks.values("seller_article"),
            warehouse__in=latest_stocks.values("warehouse"),
        ).filter(
            warehouse="Итого по складам"
        )  # Фильтруем по названию склада

...
from telegram_working.assistance import (
...
def command_reject(payment_id, user_id, reason):
    ...
    ApprovalStatus.objects.filter(
        ...
        user=ApprovedFunction.objects.get(username=user_id),
    ).update(status="REJECTED", rejection_reason=reason)
    approval_user = ApprovedFunction.objects.get(username=user_id)
    ...
    for message in messages:
        ...
        if attach == True:
            bot.edit_message_caption(
                caption=new_text,
                chat_id=chat_id_cicle,
                message_id=message_id_cicle,
                parse_mode="HTML",
            )
        else:
...
def reject_reason(update, context):
    ...
    if context.chat_data.get("last_bot_message_id"):
        ...
        if "Напишите причину отклонения заявки" in last_bot_message:
            ...
            reject_user = ApprovedFunction.objects.get(username=user_id)
            save_message_function(
                payment,
                chat_id,
                mess_id,
                "rejected_reason_inform",
                reject_user.user_name,
                update.message.text,
                "",
                False,
            )
            ...
            messages_for_delete = (
                TelegramMessageActions.objects.filter(
                    Q(payment=Payments.objects.get(id=payment_id))
                    | Q(message_type="comment_button")
                )
                .exclude(message_type="create_approve")
            ...
            message_obj = bot.send_message(
                chat_id=int(create_user.chat_id_tg),
                text=message,
                reply_to_message_id=message_rej_id,
            )
            # Записываем сообщение с причиной в базу данных
            save_message_function(
                payment,
                create_user.chat_id_tg,
            ...
            "Напишите комментарий к согласованному платежу" in last_bot_message
        ):
            chat = update.message.text
            payment_id = context.chat_data.get("payment_id")
            user_id = context.chat_data.get("user_id")
            ...
            comment_user = ApprovedFunction.objects.get(username=user_id)
...
def command_pay(context, payment_id, user_id, payment_creator, payer_company):
    ...
    pay_user = ApprovedFunction.objects.get(username=user_id)
    creator_user = ApprovedFunction.objects.get(user_name=payment_creator)
    pay = Payments.objects.get(id=payment_id)

    pay.status_of_payment = "Оплачено"
    pay.date_of_payment = now_time
    pay.payer_organization = PayerOrganization.objects.get(id=payer_company)
    if (
        str(pay.project) == "KINMAC"
        and str(pay.payer_organization)
        == "ИП Лисов Юрий Владимирович ИНН 366315065753"
        and (
            str(pay.payment_method) == "Оплата по расчетному счету"
            or str(pay.payment_method) == "Оплата по карте на сайте"
        )
    ):
        pay.payment_coefficient = 1.02
    if str(pay.project) == "KINMAC" and (
        str(pay.payment_method) == "Перевод на карту"
        or str(pay.payment_method) == "Наличная оплата"
    ):
        pay.payment_coefficient = 1.02
...
def button_click(update, context):
    ...
    if "Согласовать" in query.data:
        ...
        for payer in payers:
            ...
            button = [
                InlineKeyboardButton(
                    callback_data=data_for_button, text=payer.name
                )
            ]
        ...
        message_obj_payer = bot.send_message(
            chat_id=int(pay_user.chat_id_tg),
            text=message,
        ...
        payer_user = ApprovedFunction.objects.get(username=user_id)
        ...
        messages = TelegramMessageActions.objects.filter(
            ...
            "chat_id", "message_id", "message", "reply_markup", "attach"
        )
        ...
        for message in messages:
            ...
            new_text = (
                words[0]
                + f"‼️‼️ Статус: В работе у {pay_user_firstname} {pay_user_lastname}"
            )
            if attach == True:
                bot.edit_message_caption(
                    caption=new_text,
        ...
        keyboard = [
            [
                ...
                InlineKeyboardButton(
                    "Вернуть в очередь",
                    callback_data=f"О_тменить_В_работе {payment_id} {user_id} {payment_creator}",
                ),
                InlineKeyboardButton(
                    "Оплачено",
                    callback_data=f"Оплатить {payment_id} {user_id} {payment_creator}",
                ),
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def button_comment_handler(update, context):
    ...
    payment = Payments.objects.filter(
        Q(payment_coefficient=1), Q(payment_coefficient__gte=1)
    )
    save_message_function(
        payment[-1],
        chat_id,
    ...
    if "Не оставлять комментарий" in update.message.text:
        ...
        ).update(button_name=new_button_name)
        keyboard = ReplyKeyboardMarkup(
            [["/start"], [KeyboardButton(new_button_name)]],
    ...
    if "Необходимо прислать платёжку" in last_bot_message:
        ...
        if Payments.objects.get(id=payment_id).send_payment_file == True:
            file_path = os.path.join(
                os.getcwd(),
                "media/"
                f"{Payments.objects.get(id=payment_id).file_of_payment}",
            )
...
def command_start(update, context):
    ...
    if approved_user.rating_for_approval > 0:
        ...
        buttons = ReplyKeyboardMarkup([["/start"]], resize_keyboard=True)
    try:
    ...
    message_id = update.message.reply_text(
        "Нажмите на кнопку, чтобы создать заявку:", reply_markup=reply_markup
    ).message_id
    # message_id = update.message.message_id
...
def main():
    ...
    updater.dispatcher.add_handler(
        MessageHandler(
            Filters.text("Оставлять комментарий"), button_comment_handler
        )
    )
    updater.dispatcher.add_handler(
        MessageHandler(
            Filters.text("Не оставлять комментарий"), button_comment_handler
        )
    )
    ...
    updater.dispatcher.add_handler(
        MessageHandler(
            Filters.document | Filters.photo | Filters.video, pay_file_handler
        )
    )

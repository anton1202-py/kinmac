...
from database.models import Articles, CodingMarketplaces, CostPrice, SalesReportOnSales, WeeklyReportInDatabase
from kinmac.constants_file import BRAND_LIST, bot, TELEGRAM_ADMIN_CHAT_ID
from reklama.models import ArticleDailyCostToAdv
from sales_analytics.supplyment import calculate_storage_cost
...
def articles_analytics_data(report_number):
    ...
    costs_data = calculate_storage_cost(date_start, date_finish)
    for nm_id_dict in nm_ids_list:
        if Articles.objects.filter(nomenclatura_wb=nm_id_dict['nm_id'], brand__in=BRAND_LIST).exists():
            article = Articles.objects.get(nomenclatura_wb=nm_id_dict['nm_id'], brand__in=BRAND_LIST)

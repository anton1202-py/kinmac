...
FILTER_PLUS_DATA_LIST = [
    "Продажа",
    "Корректная продажа",
    "Сторно возвратов",
    "Авансовая оплата за товар без движения",
]
FILTER_MINUS_DATA_LIST = [
    "Сторно продаж",
    "Возврат",
    "Корректный возврат",
    "Компенсация ущерба",
...
def articles_analytics_data(report_number):
    ...
    nm_ids_list = (
        SalesReportOnSales.objects.filter(
            realizationreport_id=report_number, brand_name__in=BRAND_LIST
        )
        .values("nm_id")
        .distinct()
    )
    analytic_date = (
        SalesReportOnSales.objects.filter(realizationreport_id=report_number)
        .first()
        .date_from
    )
    analytic_date_to = (
    ...
    for nm_id_dict in nm_ids_list:
        if Articles.objects.filter(nomenclatura_wb=nm_id_dict["nm_id"]).exists():
            ...
            sale_queryset = SalesReportOnSales.objects.filter(
                realizationreport_id=report_number,
                nm_id=nm_id_dict["nm_id"],
                supplier_oper_name__in=FILTER_PLUS_DATA_LIST,
            ).aggregate(
            ...
            sale_retail_price_withdisc_rub = (
                sale_queryset["retail_price_withdisc_rub"]
                if sale_queryset["retail_price_withdisc_rub"]
                else 0
            )
            sale_retail_amount = (
                sale_queryset["retail_amount"] if sale_queryset["retail_amount"] else 0
            )
            sale_for_pay = (
                sale_queryset["ppvz_for_pay"] if sale_queryset["ppvz_for_pay"] else 0
            )
            sales_amount = (
                sale_queryset["amount_sales"] if sale_queryset["amount_sales"] else 0
            )
            return_queryset = SalesReportOnSales.objects.filter(
                realizationreport_id=report_number,
                nm_id=nm_id_dict["nm_id"],
                supplier_oper_name__in=FILTER_MINUS_DATA_LIST,
            ).aggregate(
                retail_price_withdisc_rub=Sum("retail_price_withdisc_rub"),
                retail_amount=Sum("retail_amount"),
                ppvz_for_pay=Sum("ppvz_for_pay"),
            ...
            main_penalty = SalesReportOnSales.objects.filter(
                realizationreport_id=report_number,
                nm_id=nm_id_dict["nm_id"],
            ...
            return_retail_price_withdisc_rub = (
                return_queryset["retail_price_withdisc_rub"]
                if return_queryset["retail_price_withdisc_rub"]
                else 0
            )
            return_retail_amount = (
                return_queryset["retail_amount"]
                if return_queryset["retail_amount"]
                else 0
            )
            return_returns = (
                return_queryset["ppvz_for_pay"]
                if return_queryset["ppvz_for_pay"]
                else 0
            )
            return_amount = (
                return_queryset["amount_sales"]
                if return_queryset["amount_sales"]
                else 0
            )
            sum_penalty = main_penalty["penalty"] if main_penalty["penalty"] else 0
            sum_logistic = (
                main_queryset["delivery_rub"] if main_queryset["delivery_rub"] else 0
            )
            common_sales_with_returns = sales_amount - return_amount
            realization_summ_sale = (
                sale_retail_price_withdisc_rub - return_retail_price_withdisc_rub
            )
            ...
            reimbursement_of_transportation_costs = (
                main_queryset["rebill_logistic_cost"]
                if main_queryset["rebill_logistic_cost"]
                else 0
            )
            payment_defective_and_lost = 0
            compensation_for_the_substituted = 0
            article_costprice = (
                CostPrice.objects.filter(article=article).last().costprice
                if CostPrice.objects.filter(article=article).last().costprice
                else 0
            )
            costprice_of_sales = common_sales_with_returns * article_costprice
            penalty = sum_penalty
            logistic = sum_logistic
            average_logistic_cost = (
                round((logistic / common_sales_with_returns), 2)
                if common_sales_with_returns != 0
                else 0
            )

            # Расходы на хранение
            storage = (
                costs_data.get("storage_cost") if costs_data.get("storage_cost") else 0
            )

            # Расходы на Фулфилмент
            ff_cost = (
                CostPrice.objects.filter(article=article).last().ff_cost
                if CostPrice.objects.filter(article=article).last().ff_cost
                else 0
            )
            ff_service = ff_cost * common_sales_with_returns

            # Рекламные расходы
            advertisment_cost_data = ArticleDailyCostToAdv.objects.filter(
                article=article, cost_date__range=(date_start, date_finish)
            )
            advertisment = (
                advertisment_cost_data.aggregate(cost_sum=Sum("cost"))["cost_sum"]
                if advertisment_cost_data
                else 0
            )

            refusals_and_returns_amount = return_amount
            average_percent_of_buyout = (
                round(
                    (
                        common_sales_with_returns
                        / (common_sales_with_returns + refusals_and_returns_amount)
                    )
                    * 100,
                    2,
                )
                if (common_sales_with_returns + refusals_and_returns_amount) != 0
                else 0
            )
            ...
            profit = (
                ...
                - penalty
                + reimbursement_of_transportation_costs
                + payment_defective_and_lost
                - costprice_of_sales
                - logistic
                - tax
            )
            average_profit_for_one_piece = (
                profit / common_sales_with_returns
                if common_sales_with_returns != 0
                else 0
            )
            profit_with_self_purchase = (
                average_profit_for_one_piece
                - self_purchase * average_profit_for_one_piece
            )
            roi = (
                average_profit_for_one_piece / (article_costprice + ff_service)
                if article_costprice != 0 or ff_service != 0
                else 0
            )
            profitability = (
                (profit / realization_summ_sale) * 100
                if realization_summ_sale != 0
                else 0
            )

            # Средняя цена до СПП
            average_price_before_spp = (
                realization_summ_sale / common_sales_with_returns
                if common_sales_with_returns
                else 0
            )
            if ArticleSaleAnalytic.objects.filter(
                article=article, sales_report_number=report_number
            ).exists():
            ...
            print(
                f'В базе нет артикула: {nm_id_dict["nm_id"]} в отчете {report_number}'
            )

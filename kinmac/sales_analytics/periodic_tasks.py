...
@app.task
def commom_analytics_data(datestart=None, datefinish=None, article_filter=None):
    """Обновляет общие данные аналитики из аналитика каждого артикула"""
    CommonSaleAnalytic.objects.all().delete()
    articles_list = Articles.objects.filter(brand__in=BRAND_LIST)
    if article_filter:
        articles_list = Articles.objects.filter(common_article=article_filter)
    for article in articles_list:
        if ArticleSaleAnalytic.objects.filter(article=article).exists():
            ...
            if datestart:
                analytic_queryset = ArticleSaleAnalytic.objects.filter(article=article, analytic_date__gte=datestart)
            if datefinish:
                analytic_queryset = ArticleSaleAnalytic.objects.filter(article=article, analytic_date__lte=datefinish)
            
            if analytic_queryset:
                analytic_queryset = analytic_queryset.aggregate(
